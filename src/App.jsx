...
// LoginPage removed as per request

function App() {
  // Auth State
  const [user, setUser] = useState({ isGuest: true, displayName: 'Public User' });
  const [authLoading, setAuthLoading] = useState(false); // No longer loading auth as we use default guest
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        ...
        setUser({ isGuest: true, displayName: 'Public User' });
      }
    ...
    <div className="h-screen w-full bg-gray-900 text-gray-100 flex flex-col overflow-hidden">
      ...
      <header className="h-14 border-b border-gray-800 flex items-center justify-between px-6 bg-gray-900 shrink-0">
        <div className="flex items-center gap-2 text-primary">
          <Cpu size={24} />
          <h1 className="text-xl font-bold tracking-tight">AI Chess Analysis & Engine Coach</h1>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                ...
                {showLoadGameTray && (
                  <div className="p-4 border-t border-gray-800 space-y-2">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="text-xs font-semibold text-gray-500 uppercase">Analyze New Game</h3>
          ...
          {/* Center Panel: Board */}
          <div className="flex-1 bg-gray-950 flex flex-col items-center justify-center p-2 sm:p-4 overflow-auto">
            <div className="w-full h-full flex flex-col items-center justify-center">
              <div
                className="w-full max-w-[min(400px,90vw,90vh)] sm:max-w-[min(500px,90vw,90vh)] shadow-2xl shadow-black/50 rounded-lg bg-[#b58863] relative aspect-square"
              >
                {/* Use Custom Wrapper */}
                <ChessboardJS
                  fen={fen}
              ...
              {appMode !== 'puzzle' && !playMode && (
                <div className="mt-4 sm:mt-6 w-full max-w-[min(400px,90vw,90vh)] sm:max-w-[min(500px,90vw,90vh)] flex flex-col gap-2">
          ...
          <div className="w-full md:w-96 border-l border-gray-800 flex flex-col bg-gray-900/50">
            <Card className="h-1/3 border-0 rounded-none border-b border-gray-800" title="Deep Chess Analysis" icon={AlertTriangle}>
            ...
            <Card className="flex-1 border-0 rounded-none flex flex-col" title="AI Chess Coach & Mentor" icon={MessageSquare}>
      ...
      {/* SEO Footer */}
      <footer className="h-12 border-t border-gray-800 bg-gray-900 px-6 flex items-center justify-between text-[10px] text-gray-500 shrink-0">
        <div className="flex flex-col">
          <p><span className="font-semibold text-gray-400 uppercase tracking-wider">AI Chess Analysis</span> &mdash; The ultimate tool for deep chess game analysis using Stockfish 16.1 and Google Gemini AI.</p>
          <p>Improve your opening, middle-game, and endgame with real-time engine evaluation and personalized AI coaching.</p>
        </div>
        <div className="hidden md:flex gap-4 uppercase tracking-tighter">
          <span>Free Online Chess Analyzer</span>
          <span className="text-gray-700">|</span>
          <span>Stockfish Analysis</span>
          <span className="text-gray-700">|</span>
          <span>Analyze PGNS Online</span>
        </div>
      </footer>
